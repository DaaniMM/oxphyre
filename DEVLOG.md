...
- `public/js/tour-viewer.js`: ajuste local Fase 1.1 tras prueba visual. El layout pasa a recuperar sensacion inmersiva: 1 foto centrada, 2 fotos a -75/+75 grados, 3 fotos repartidas en 360 grados a 0/120/240 y 4 fotos a 0/90/180/270. Los paneles se acercan ligeramente a la camara y aumentan su altura/ancho maximo segun ratio, manteniendo la proporcion original de cada textura.
- `public/js/tour-viewer.js`: ajuste local Fase 1.2 para aumentar presencia visual sin cambiar layout ni deformar. `getRoomPanelDimensions()` acerca ligeramente los paneles (`radius=4.05`) y aumenta alturas/ancho maximo por tipo: horizontales mas amplias, verticales mas altas como tarjeta y neutras intermedias. `widthAngle` sigue calculandose desde `height * aspect / radius`, por lo que se conserva el ratio original.
- `public/js/tour-viewer.js`: ajuste local Fase 1.2b tras prueba en produccion. Se redujo la presencia de paneles para que no invadan visualmente el aro inferior: `radius=4.22`, alturas/ancho maximo intermedios frente a Fase 1.2 y boost de 1 foto mas moderado. Se mantiene mas presencia que Fase 1.1 y se conserva `widthAngle = height * aspect / radius`.
