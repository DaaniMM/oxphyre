...
- En `businesses`, al borrar logicamente se libera el slug activo renombrando el registro soft deleted con sufijo interno `-deleted-{id}`. Validado: `negocioofree` paso a `negocioofree-deleted-2` y el slug `negocioofree` pudo reutilizarse sin error 500.
...
- Estado implementado validado: una sola marca central diagonal "OXPHYRE" + badge "Creado con Oxphyre" hacia `/precios`; no bloquea drag ni hotspots/flechas y se mantiene al navegar por hotspot.
